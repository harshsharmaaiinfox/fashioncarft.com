...
if ( ! function_exists( 'dunker_add_rest_api_pagination_global_variables' ) ) {
	/**
	 ...
	 * @return array
	 */
	function dunker_add_rest_api_pagination_global_variables( $global, $namespace ) {
		$global['paginationRestRoute'] = $namespace . '/get-posts';

		return $global;
	}

	add_filter( 'dunker_filter_rest_api_global_variables', 'dunker_add_rest_api_pagination_global_variables', 10, 2 );
}

if ( ! function_exists( 'dunker_add_rest_api_pagination_route' ) ) {
	/**
	 ...
	 * @return array
	 */
	function dunker_add_rest_api_pagination_route( $routes ) {
		$routes['pagination'] = array(
			'route'    => 'get-posts',
			'methods'  => WP_REST_Server::READABLE,
			'callback' => 'dunker_get_new_posts',
			'args'     => array(
				'options' => array(
					'required'          => true,
					...
					'description'       => esc_html__( 'Options data is array with all selected shortcode parameters value', 'dunker' ),
				),
			),
		);
	...
	add_filter( 'dunker_filter_rest_api_routes', 'dunker_add_rest_api_pagination_route' );
}

if ( ! function_exists( 'dunker_get_new_posts' ) ) {
	/**
	 * Function that load new posts for pagination functionality
	 *
	 * @return void
	 */
	function dunker_get_new_posts() {

		if ( ! isset( $_GET ) || empty( $_GET ) ) {
			dunker_get_ajax_status( 'error', esc_html__( 'Get method is invalid', 'dunker' ) );
		} else {
			...
			if ( ! empty( $options ) ) {
				...
				$query_args = dunker_get_query_params( $options );
				...
				echo apply_filters( "dunker_filter_{$get_template_part}", $get_template_part( $module . '/' . $shortcode, 'templates/loop', '', $options ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				...
				if ( 'standard' === $options['pagination_type'] ) {
					$pagination_html = apply_filters( "dunker_filter_{$get_template_part}_pagination", dunker_get_template_part( 'pagination', 'templates/pagination', $options['pagination_type'], $options ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}

				$data = apply_filters(
					'dunker_filter_pagination_data_return',
					array(
						'html'            => $html,
				...
				dunker_get_ajax_status( 'success', esc_html__( 'Items are loaded', 'dunker' ), $data );
			} else {
				dunker_get_ajax_status( 'error', esc_html__( 'Options are invalid', 'dunker' ) );
			}
		}
	}
}

if ( ! function_exists( 'dunker_get_query_params' ) ) {
	/**
	 ...
	 * @return array
	 */
	function dunker_get_query_params( $params ) {
		...
		return apply_filters( 'dunker_filter_query_params', $args, $params );
	}
}

if ( ! function_exists( 'dunker_get_pagination_data' ) ) {
	/**
	 ...
	 * @return array
	 */
	function dunker_get_pagination_data( $plugin, $module, $shortcode, $post_type, $params ) {
		$data = array();
		...
		if ( ! empty( $post_type ) && ! empty( $params ) ) {
			...
			if ( isset( $params['space'] ) && ! empty( $params['space'] ) ) {
				$params['space_value'] = dunker_get_space_value( $params['space'] );
			}
...
if ( ! function_exists( 'dunker_add_link_pages_after_content' ) ) {
	/**
	 * Function which add pagination for blog single and page
	 */
	function dunker_add_link_pages_after_content() {

		$args_pages = array(
			'before'      => '<div class="qodef-single-links qodef-m"><span class="qodef-m-single-links-title">' . esc_html__( 'Pages: ', 'dunker' ) . '</span>',
			'after'       => '</div>',
			'link_before' => '<span>',
			'link_after'  => '</span>',
			'pagelink'    => '%',
	...
	add_action( 'dunker_action_after_blog_single_content', 'dunker_add_link_pages_after_content' );
	add_action( 'dunker_action_after_page_content', 'dunker_add_link_pages_after_content' );
}
