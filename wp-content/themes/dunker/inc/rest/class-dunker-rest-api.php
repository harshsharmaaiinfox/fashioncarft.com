...
if ( ! class_exists( 'Dunker_Rest_API' ) ) {
	/**
	 * Rest API class with configuration
	 */
	class Dunker_Rest_API {
		private static $instance;
		private $namespace;

		public function __construct() {
			// Init variables
			$this->set_namespace( 'dunker/v1' );

			// Localize theme's main js script with rest variable
			add_filter( 'dunker_filter_localize_main_js', array( $this, 'localize_script' ) );
		...
		function localize_script( $global ) {
			...
			return apply_filters( 'dunker_filter_rest_api_global_variables', $global, $this->get_namespace() );
		}

		function register_rest_api_route() {
			$routes = apply_filters( 'dunker_filter_rest_api_routes', array() );
