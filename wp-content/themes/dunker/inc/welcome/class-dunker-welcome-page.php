...
if ( ! class_exists( 'Dunker_Welcome_Page' ) ) {
	/**
	 * Welcome page theme class
	 */
	class Dunker_Welcome_Page {
		private static $instance;

		/**
		 * @return Dunker_Welcome_Page
		 */
		...
		function init_activation_hook() {
			if ( ! is_network_admin() ) {
				set_transient( '_dunker_welcome_page_redirect', 1, 30 );
			}
		}
		...
		function welcome_page_redirect() {
			...
			if ( ! get_transient( '_dunker_welcome_page_redirect' ) ) {
				return;
			}

			// delete the redirect transient
			delete_transient( '_dunker_welcome_page_redirect' );
			...
			wp_safe_redirect( add_query_arg( array( 'page' => 'dunker_welcome_page' ), esc_url( admin_url( 'themes.php' ) ) ) );
			exit;
		}
		...
		function create_welcome_page() {
			...
			remove_submenu_page( 'themes.php', 'dunker_welcome_page' );
		}
		...
		function welcome_page_content() {
			...
			$params['theme_screenshot']  = file_exists( DUNKER_ROOT_DIR . '/screenshot.png' ) ? DUNKER_ROOT . '/screenshot.png' : DUNKER_ROOT . '/screenshot.jpg';

			dunker_template_part( 'welcome', 'templates/welcome', '', $params );
		}
		...
		function enqueue_styles( $hook ) {
			...
			if ( 'appearance_page_dunker_welcome_page' === $hook ) {
				wp_enqueue_style( 'dunker-welcome-page-style', DUNKER_INC_ROOT . '/welcome/assets/admin/css/welcome.min.css' );
			}
		}
	}

	Dunker_Welcome_Page::get_instance();
}
