...
if ( ! function_exists( 'dunker_get_search_page_excerpt_length' ) ) {
	/**
	 * Function that return number of characters for excerpt on search page
	 *
	 * @return int
	 */
	function dunker_get_search_page_excerpt_length() {
		$length = apply_filters( 'dunker_filter_post_excerpt_length', 180 );

		return intval( $length );
	}
}

if ( ! function_exists( 'dunker_override_search_block_templates' ) ) {
	/**
	 * Function that override `core/search` block template
	 *
	 * @see register_block_core_search()
	 */
	function dunker_override_search_block_templates( $atts ) {
		if ( ! empty( $atts ) && isset( $atts['render_callback'] ) && 'render_block_core_search' === $atts['render_callback'] && function_exists( 'styles_for_block_core_search' ) ) {
			$atts['render_callback'] = 'dunker_render_block_core_search';
		}

		return $atts;
	}

	add_filter( 'block_type_metadata_settings', 'dunker_override_search_block_templates' );
}

if ( ! function_exists( 'dunker_render_block_core_search' ) ) {
	/**
	 ...
	 */
	function dunker_render_block_core_search( $attributes ) {
		static $instance_id = 0;

		$attributes = wp_parse_args(
			$attributes,
			array(
				'label'      => esc_html__( 'Search', 'dunker' ),
				'buttonText' => esc_html__( 'Search', 'dunker' ),
			)
		);
		...
		$label_markup = sprintf(
			...
			$input_id,
			empty( $attributes['label'] ) ? esc_html__( 'Search', 'dunker' ) : esc_html( $attributes['label'] )
		);
		...
		if ( $show_button ) {
			...
			if ( ! $use_icon_button ) {
				...
				$button_internal_markup = dunker_get_svg_icon( 'search' );
			}
