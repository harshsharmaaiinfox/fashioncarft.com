...
if ( $has_image ) {
	$portfolio_url       = dunker_core_get_portfolio_list_item_url( get_the_ID() );
	...
	$attr = array();
	if ( 'yes' === $disable_lazy_loading ) {
		$attr['loading'] = 'eager';
	}
	?>
	<div class="qodef-e-media-image">
		<a itemprop="url" href="<?php echo esc_url( $portfolio_url['link'] ); ?>" target="<?php echo esc_attr( $portfolio_url['target'] ); ?>">
			<?php echo dunker_core_get_list_shortcode_item_image( $image_dimension, $portfolio_list_image, $custom_image_width, $custom_image_height, $attr ); ?>
		</a>
	</div>
<?php } ?>
