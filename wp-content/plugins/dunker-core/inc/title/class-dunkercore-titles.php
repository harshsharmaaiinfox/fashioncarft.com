...
class DunkerCore_Titles {
	...
	public function __construct() {
		...
		add_filter( 'dunker_filter_page_title_classes', array( $this, 'add_page_title_classes' ) );

		// Add title area inline styles
		add_filter( 'dunker_filter_add_inline_style', array( $this, 'add_inline_styles' ) );
	}

	/**
	 * @return DunkerCore_Titles
	 */
	...
	function include_layouts() {
		...
		foreach ( glob( DUNKER_CORE_INC_PATH . '/title/dashboard/*/*.php' ) as $admin ) {
			include_once $admin;
		}

		foreach ( glob( DUNKER_CORE_INC_PATH . '/title/layouts/*/include.php' ) as $layout ) {
			include_once $layout;
		}
	}

	function set_variables() {
		$layout_meta = dunker_core_get_post_value_through_levels( 'qodef_title_layout' );
		$layouts     = apply_filters( 'dunker_core_filter_register_title_layouts', $layouts = array() );
	...
	function render_template() {
		...
		if ( ! empty( $title_object ) ) {
			$template_hook = $title_object->overriding_whole_title ? 'dunker_filter_title_template' : 'dunker_filter_title_content_template';
	...
	function add_page_title_classes( $classes ) {
		$layout             = dunker_core_get_post_value_through_levels( 'qodef_title_layout' );
		$text_alignment     = dunker_core_get_post_value_through_levels( 'qodef_page_title_text_alignment' );
		$vertical_alignment = dunker_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$image_params       = dunker_core_get_page_title_image_params();
	...
	function add_inline_styles( $style ) {
		...
		$height                   = dunker_core_get_post_value_through_levels( 'qodef_page_title_height' );
		$height_mobile            = dunker_core_get_post_value_through_levels( 'qodef_page_title_height_on_smaller_screens' );
		$title_padding            = apply_filters( 'dunker_core_filter_title_padding', 0 );
		$title_padding_mobile     = apply_filters( 'dunker_core_filter_title_padding_mobile', 0 );
		$title_vertical_alignment = dunker_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$background_color         = dunker_core_get_post_value_through_levels( 'qodef_page_title_background_color' );
		$image_params             = dunker_core_get_page_title_image_params();
		...
		$title_color = dunker_core_get_post_value_through_levels( 'qodef_page_title_color' );
...
DunkerCore_Titles::get_instance();
