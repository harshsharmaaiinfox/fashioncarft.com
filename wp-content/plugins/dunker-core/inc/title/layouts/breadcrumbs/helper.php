...
if ( ! function_exists( 'dunker_core_register_breadcrumbs_title_layout' ) ) {
	/**
	 ...
	 * @return array
	 */
	function dunker_core_register_breadcrumbs_title_layout( $layouts ) {
		$layouts['breadcrumbs'] = 'DunkerCore_Breadcrumbs_Title';

		return $layouts;
	}

	add_filter( 'dunker_core_filter_register_title_layouts', 'dunker_core_register_breadcrumbs_title_layout' );
}

if ( ! function_exists( 'dunker_core_add_breadcrumbs_title_layout_option' ) ) {
	/**
	 ...
	 * @return array
	 */
	function dunker_core_add_breadcrumbs_title_layout_option( $layouts ) {
		$layouts['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'dunker-core' );

		return $layouts;
	}

	add_filter( 'dunker_core_filter_title_layout_options', 'dunker_core_add_breadcrumbs_title_layout_option' );
}

if ( ! function_exists( 'dunker_core_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs html
	 */
	function dunker_core_breadcrumbs() {
		$page_id = qode_framework_get_page_id();

		// Breadcrumbs label
		$labels = apply_filters(
			'dunker_core_filter_breadcrumbs_label',
			array(
				'home'        => esc_html__( 'Home', 'dunker-core' ),
				'tag'         => esc_html__( 'Posts tagged "%s"', 'dunker-core' ),
				'author'      => esc_html__( 'Posted by %s', 'dunker-core' ),
				'search'      => esc_html__( 'Search results for "%s"', 'dunker-core' ),
				'404'         => esc_html__( '404 - Page not found', 'dunker-core' ),
				'query_paged' => esc_html__( '(Page %s)', 'dunker-core' ),
			)
		);

		// Breadcrumbs variables
		$settings = apply_filters(
			'dunker_core_filter_breadcrumbs_settings',
			array(
		...
		$breadcrumbs_html = '';
		if ( ! empty( $wrap ) ) {
			$breadcrumbs_html = $settings['wrap_before'] . $wrap . apply_filters( 'dunker_core_filter_breadcrumbs_content', $wrap_child, $settings ) . $settings['wrap_after'];
		}

		echo apply_filters( 'dunker_core_filter_breadcrumbs_template', $breadcrumbs_html );
	}
}
