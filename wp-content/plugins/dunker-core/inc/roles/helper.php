...
if ( ! function_exists( 'dunker_core_include_role_custom_fields' ) ) {
	/**
	 * Function that includes role custom fields files
	 */
	function dunker_core_include_role_custom_fields() {
		foreach ( glob( DUNKER_CORE_INC_PATH . '/roles/*/role-fields.php' ) as $role_fields ) {
			include_once $role_fields;
		}
	}

	add_action( 'qode_framework_action_custom_user_fields', 'dunker_core_include_role_custom_fields' );
}

if ( ! function_exists( 'dunker_core_register_role_custom_fields' ) ) {
	/**
	 * Function that registers role custom fields files
	 */
	function dunker_core_register_role_custom_fields() {
		do_action( 'dunker_core_action_register_role_custom_fields' );
	}

	add_action( 'qode_framework_action_custom_user_fields', 'dunker_core_register_role_custom_fields', 11 );
}

if ( ! function_exists( 'dunker_core_profile_get_social_params' ) ) {
	/**
	 * Returns owner params
	 */
	function dunker_core_profile_get_social_params( $params ) {
	...
	add_filter( 'dunker_membership_filter_user_params', 'dunker_core_profile_get_social_params' );
}

if ( ! function_exists( 'dunker_core_social_edit_profile_fields' ) ) {
	function dunker_core_social_edit_profile_fields( $page, $params ) {
		$roles_social_scope = apply_filters( 'dunker_core_filter_role_social_array', array( 'administrator', 'author' ) );
		$enable             = false;
	...
	add_action( 'dunker_membership_action_after_dashboard_edit_profile_fields', 'dunker_core_social_edit_profile_fields', 15, 2 );
}

if ( ! function_exists( 'dunker_core_user_profile_social_update' ) ) {
	function dunker_core_user_profile_social_update( $options, $user_id ) {
	...
	add_action( 'dunker_membership_action_update_user_profile', 'dunker_core_user_profile_social_update', 10, 2 );
}
