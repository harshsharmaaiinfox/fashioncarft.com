...
if ( ! function_exists( 'dunker_core_get_header_logo_image' ) ) {
	/**
	 * This function print header logo image
	 *
	 * @param array $parameters
	 */
	function dunker_core_get_header_logo_image( $parameters = array() ) {
		$header_skin     = dunker_core_get_post_value_through_levels( 'qodef_header_skin' );
		$logo_height     = dunker_core_get_post_value_through_levels( 'qodef_logo_height' );
		$logo_source     = dunker_core_get_post_value_through_levels( 'qodef_logo_source' );
		$logo_svg_path   = dunker_core_get_post_value_through_levels( 'qodef_logo_svg_path' );
		$logo_text       = dunker_core_get_post_value_through_levels( 'qodef_logo_text' );
		$customizer_logo = dunker_core_get_customizer_logo();
		...
		if ( 'svg-path' === $logo_source && ! empty( $logo_svg_path ) ) {
			$logo_html['logo_main_image'] = apply_filters( 'dunker_core_filter_header_logo_svg_path', $logo_svg_path, $parameters );

			$is_enabled = true;
		} elseif ( 'textual' === $logo_source && ! empty( $logo_text ) ) {
			$logo_html['logo_main_image'] = esc_html( apply_filters( 'dunker_core_filter_header_logo_textual', $logo_text, $parameters ) );

			$is_enabled = true;
		} else {
			$available_logos = apply_filters(
				'dunker_core_filter_available_header_logo_images',
				array(
					'main'  => 'main',
					'dark'  => 'dark',
					'light' => 'light',
			...
			foreach ( $available_logos as $logo_key => $option_value ) {
				...
				$logo_image_id = dunker_core_get_post_value_through_levels( 'qodef_logo_' . $option_value );

				if ( empty( $logo_image_id ) && ! empty( $header_skin ) ) {
					$logo_image_id = dunker_core_get_post_value_through_levels( 'qodef_logo_main' );
				}
				...
				if ( ! empty( $logo_image_id ) ) {
					$logo_image_attr = array(
						...
						'itemprop' => 'image',
						'alt'      => sprintf( esc_attr__( 'logo %s', 'dunker-core' ), str_replace( '_', ' ', $logo_key ) ),
					);
		...
		$parameters['logo_image'] = implode( '', apply_filters( 'dunker_core_filter_header_logo_image_html', $logo_html, $parameters ) );

		if ( $is_enabled ) {
			echo apply_filters( 'dunker_core_filter_get_header_logo_image', dunker_core_get_template_part( 'header/templates', 'parts/logo', '', $parameters ), $parameters );
		}
	}
}

if ( ! function_exists( 'dunker_core_set_header_logo_styles' ) ) {
	/**
	 ...
	 * @return string
	 */
	function dunker_core_set_header_logo_styles( $style ) {
		$logo_styles  = array();
		$logo_height  = dunker_core_get_post_value_through_levels( 'qodef_logo_height' );
		$logo_padding = dunker_core_get_post_value_through_levels( 'qodef_logo_padding' );
		...
		// Logo SVG Source
		$svg_styles     = array();
		$svg_icon_color = dunker_core_get_post_value_through_levels( 'qodef_logo_svg_path_color' );
		...
		$svg_icon_size   = dunker_core_get_post_value_through_levels( 'qodef_logo_svg_path_size' );
		...
		$svg_icon_hover_color = dunker_core_get_post_value_through_levels( 'qodef_logo_svg_path_hover_color' );
		...
		// Logo Textual Source
		$textual_styles       = dunker_core_get_typography_styles( 'qodef_logo_text', '', null );
		$textual_hover_styles = dunker_core_get_typography_hover_styles( 'qodef_logo_text', null );
	...
	add_filter( 'dunker_filter_add_inline_style', 'dunker_core_set_header_logo_styles' );
}

if ( ! function_exists( 'dunker_core_get_header_widget_area' ) ) {
	/**
	 ...
	 * @param bool $enabled
	 */
	function dunker_core_get_header_widget_area( $widget_area = 'one', $widget_area_slug = 'header-widget-area', $custom_widget_area_slug = '', $enabled = false ) {
		...
		if ( $is_enabled || $enabled ) {
			...
			$parameters = apply_filters(
				'dunker_core_filter_header_widget_area',
				array(
					'page_id'             => $page_id,
			...
			dunker_core_template_part( 'header/templates', 'parts/widgets', '', $parameters );
		}
	}
}
