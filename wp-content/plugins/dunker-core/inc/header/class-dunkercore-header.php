...
abstract class DunkerCore_Header {
	...
	public function __construct() {
		...
		add_action( 'dunker_core_action_before_main_css', array( $this, 'enqueue_assets' ) );
		add_action( 'dunker_core_action_before_main_css', array( $this, 'enqueue_additional_assets' ) );
		add_filter( 'dunker_filter_localize_main_js', array( $this, 'set_global_javascript_variables' ) );
		add_filter( 'dunker_core_filter_content_margin', array( $this, 'get_content_margin' ) );
		add_filter( 'dunker_core_filter_title_padding', array( $this, 'get_title_padding' ) );
		add_filter( 'dunker_filter_add_inline_style', array( $this, 'set_inline_header_styles' ) );
		add_filter( 'dunker_filter_header_inner_class', array( $this, 'set_header_inner_classes' ), 10, 2 );
		add_filter( 'dunker_core_filter_nav_menu_header_selector', array( $this, 'set_nav_menu_header_selector' ) );
		add_filter( 'dunker_core_filter_nav_menu_narrow_header_selector', array( $this, 'set_nav_menu_narrow_header_selector' ) );
	...
	public function load_template( $parameters = array() ) {
		$parameters = apply_filters( 'dunker_core_filter_header_template', $parameters );

		return dunker_core_get_template_part( 'header/layouts/' . $this->get_layout(), 'templates/' . $this->get_layout(), $this->get_layout_slug(), $parameters );
	}
	...
	public function get_header_transparency() {
		$background_color = dunker_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_background_color' );
	...
	public function content_behind_header() {
		return 'yes' === dunker_core_get_post_value_through_levels( 'qodef_content_behind_header' );
	}
	...
	function set_header_height() {
		$header_height = dunker_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_height' );
		$header_height = ! empty( $header_height ) ? intval( $header_height ) : $this->default_header_height;

		$this->header_height = apply_filters( 'dunker_core_filter_set_header_height', $header_height );
	}

	function set_header_inner_classes( $class, $layout ) {
		// Check is content in grid
		$class[] = 'yes' === dunker_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_in_grid' ) ? 'qodef-content-grid' : '';

		// Check header skin
		$header_skin = dunker_core_get_post_value_through_levels( 'qodef_header_skin' );
	...
	public function set_inline_header_styles( $style ) {
		...
		$height           = dunker_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_height' );
		$background_color = dunker_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_background_color' );
		...
		$side_padding = dunker_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_side_padding' );
		$border_color = dunker_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_border_color' );
		$border_width = dunker_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_border_width' );
		$border_style = dunker_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_border_style' );
		...
		if ( ! empty( $border_color ) && ($this->get_formatted_layout() !== 'vertical_sliding') ) {
			$inner_styles['border-bottom-color'] = $border_color;

			if ( empty( $border_width ) ) {
				$inner_styles['border-bottom-width'] = '1px';
			}
		} else if (! empty( $border_color ) && ($this->get_formatted_layout() === 'vertical_sliding') ) {
		...
		if ( ! empty( $border_width ) && ($this->get_formatted_layout() !== 'vertical_sliding') ) {
			$inner_styles['border-bottom-width'] = intval( $border_width ) . 'px';
		} else if ( ! empty( $border_width ) && ($this->get_formatted_layout() === 'vertical_sliding') ) {
            $inner_styles['border-right-width'] = intval( $border_width ) . 'px';
        }

		if ( ! empty( $border_style ) && ($this->get_formatted_layout() !== 'vertical_sliding') ) {
