...
class DunkerCore_Top_Area {
	...
	public function __construct() {
		...
		add_action( 'dunker_action_before_page_header', array( $this, 'load_template' ) );
		...
		add_filter( 'dunker_filter_add_inline_style', array( $this, 'set_inline_top_area_styles' ) );
		add_filter( 'dunker_core_filter_top_area_inner_class', array( $this, 'set_grid_class' ) );

		add_action( 'dunker_action_before_page_header',  array( $this, 'dunker_core_enable_top_message' ), '5');
	}

	/**
	 * @return DunkerCore_Top_Area
	 */
	...
	function is_top_area_enabled() {
		$top_area_enabled = 'yes' === dunker_core_get_post_value_through_levels( 'qodef_top_area_header' );
	...
	public function set_top_area_height() {
		if ( $this->is_top_area_enabled ) {
			$height         = dunker_core_get_post_value_through_levels( 'qodef_top_area_header_height' );
	...
	function dunker_core_enable_top_message() {
		$enable_top_message = 'yes' === dunker_core_get_post_value_through_levels( 'qodef_top_area_message_header' );
		$top_message = dunker_core_get_option_value( 'admin', 'qodef_top_area_message' );
		$top_message_skin = 'qodef-skin--' . dunker_core_get_post_value_through_levels( 'qodef_top_area_message_skin' );
		$top_message_link = dunker_core_get_option_value( 'admin', 'qodef_top_area_message_link' );
		if ( $enable_top_message ) {
			?>
			<div id="qodef-top-message-holder" <?php qode_framework_class_attribute( $top_message_skin ); ?>>
				<div class="qodef-top-message-inner">
					...
					<span class="qodef-close-message">
						<?php dunker_core_render_svg_icon('close'); ?>
                    </span>
				</div>
			</div>
		<?php }
	...
	public function is_top_area_transparent() {
		$background_color = dunker_core_get_post_value_through_levels( 'qodef_top_area_header_background_color' );
	...
	public function load_template() {
		...
		dunker_core_template_part( 'header/top-area/', 'templates/top-area', '', $parameters );
	}
	...
	public function set_inline_top_area_styles( $style ) {
		...
		$height       = dunker_core_get_post_value_through_levels( 'qodef_top_area_header_height' );
		$side_padding = dunker_core_get_post_value_through_levels( 'qodef_top_area_header_side_padding' );
		$border_color = dunker_core_get_post_value_through_levels( 'qodef_top_area_header_border_color' );
		$border_width = dunker_core_get_post_value_through_levels( 'qodef_top_area_header_border_width' );
		$border_style = dunker_core_get_post_value_through_levels( 'qodef_top_area_header_border_style' );
	...
	public function set_grid_class( $classes ) {
		if ( 'yes' === dunker_core_get_post_value_through_levels( 'qodef_top_area_header_in_grid' ) ) {
...
DunkerCore_Top_Area::get_instance();
