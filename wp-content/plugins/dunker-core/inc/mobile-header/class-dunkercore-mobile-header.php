...
abstract class DunkerCore_Mobile_Header {
	...
	public function __construct() {
		...
		add_filter( 'dunker_core_filter_nav_menu_mobile_header_selector', array( $this, 'set_nav_menu_header_selector' ) );
		add_filter( 'dunker_filter_mobile_header_inner_class', array( $this, 'set_mobile_header_inner_classes' ), 10, 2 );
		add_filter( 'dunker_filter_add_inline_style', array( $this, 'set_inline_mobile_header_styles' ) );
		add_filter( 'dunker_core_filter_content_margin_mobile', array( $this, 'get_content_margin' ) );
		add_filter( 'dunker_core_filter_title_padding_mobile', array( $this, 'get_title_padding' ) );
	...
	public function load_template( $parameters = array() ) {
		$parameters = apply_filters( 'dunker_core_filter_mobile_header_template', $parameters );

		return dunker_core_get_template_part( 'mobile-header/layouts/' . $this->get_layout(), 'templates/' . $this->get_layout(), $this->get_layout_slug(), $parameters );
	}
	...
	function set_mobile_header_inner_classes( $class, $layout ) {
		...
		$class[] = 'yes' === dunker_core_get_post_value_through_levels( 'qodef_mobile_header_in_grid' ) ? 'qodef-content-grid' : '';
	...
	public function set_inline_mobile_header_styles( $style ) {
		...
		$height           = dunker_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_mobile_header_height' );
		$background_color = dunker_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_mobile_header_background_color' );
        $border_color     = dunker_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_mobile_header_border_color' );
        $border_width     = dunker_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_mobile_header_border_width' );
        $border_style     = dunker_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_mobile_header_border_style' );
        ...
        if ( ! empty( $border_color ) ) {
            $item_styles['border-bottom-color'] = $border_color;

            if ( empty( $border_width ) ) {
                $item_styles['border-bottom-width'] = '1px';
            }
        }

        if ( ! empty( $border_width ) ) {
            $item_styles['border-bottom-width'] = intval( $border_width ) . 'px';
        }

        if ( ! empty( $border_style ) ) {
            $item_styles['border-bottom-style'] = $border_style;
        }
		...
		$side_padding = dunker_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_mobile_header_side_padding' );
	...
	function set_header_height() {
		$header_height = dunker_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_mobile_header_height' );
		$header_height = ! empty( $header_height ) ? intval( $header_height ) : $this->default_header_height;

		$this->header_height = apply_filters( 'dunker_core_filter_set_mobile_header_height', $header_height );
	}
