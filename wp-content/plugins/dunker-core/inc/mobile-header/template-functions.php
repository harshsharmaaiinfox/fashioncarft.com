...
if ( ! function_exists( 'dunker_core_get_mobile_header_logo_image' ) ) {
	/**
	 * Function that return logo image html for current module
	 *
	 * @return string that contains html content
	 */
	function dunker_core_get_mobile_header_logo_image() {
		$logo_height     = dunker_core_get_post_value_through_levels( 'qodef_mobile_logo_height' );
		$logo_source     = dunker_core_get_post_value_through_levels( 'qodef_mobile_logo_source' );
		$main_logo_id    = dunker_core_get_post_value_through_levels( 'qodef_mobile_logo_main' );
		$logo_svg_path   = dunker_core_get_post_value_through_levels( 'qodef_mobile_logo_svg_path' );
		$logo_text       = dunker_core_get_post_value_through_levels( 'qodef_mobile_logo_text' );
		...
		$available_logos = apply_filters(
			'dunker_core_filter_available_mobile_header_logo_images',
			array(
				'main' => 'main',
			),
			$parameters
		...
		if ( 'svg-path' === $logo_source && ! empty( $logo_svg_path ) ) {
			$logo_html['logo_main_image'] = apply_filters( 'dunker_core_filter_mobile_header_logo_svg_path', $logo_svg_path, $parameters );

			$is_enabled = true;
		} elseif ( 'textual' === $logo_source && ! empty( $logo_text ) ) {
			$logo_html['logo_main_image'] = esc_html( apply_filters( 'dunker_core_filter_mobile_header_logo_textual', $logo_text, $parameters ) );
			...
			foreach ( $available_logos as $logo_key => $option_value ) {
				...
				$logo_image_id = dunker_core_get_post_value_through_levels( 'qodef_mobile_logo_' . $option_value );
		...
		$parameters['logo_image'] = implode( '', apply_filters( 'dunker_core_filter_mobile_header_logo_image_html', $logo_html, $parameters ) );

		if ( $is_enabled ) {
			echo apply_filters( 'dunker_core_filter_get_mobile_header_logo_image', dunker_core_get_template_part( 'mobile-header/templates', 'parts/mobile-logo', '', $parameters ), $parameters, $logo_html );
		}
	}
}

if ( ! function_exists( 'dunker_core_set_mobile_header_logo_styles' ) ) {
	/**
	 ...
	 * @return string
	 */
	function dunker_core_set_mobile_header_logo_styles( $style ) {
		$logo_styles  = array();
		$logo_height  = dunker_core_get_post_value_through_levels( 'qodef_mobile_logo_height' );
		$logo_padding = dunker_core_get_post_value_through_levels( 'qodef_mobile_logo_padding' );
		...
		// Logo SVG Source
		$svg_styles     = array();
		$svg_icon_color = dunker_core_get_post_value_through_levels( 'qodef_mobile_logo_svg_path_color' );
		...
		$svg_icon_size   = dunker_core_get_post_value_through_levels( 'qodef_mobile_logo_svg_path_size' );
		...
		$svg_icon_hover_color = dunker_core_get_post_value_through_levels( 'qodef_mobile_logo_svg_path_hover_color' );
		...
		// Logo Textual Source
		$textual_styles       = dunker_core_get_typography_styles( 'qodef_mobile_logo_text', '', null );
		$textual_hover_styles = dunker_core_get_typography_hover_styles( 'qodef_mobile_logo_text', null );
	...
	add_filter( 'dunker_filter_add_inline_style', 'dunker_core_set_mobile_header_logo_styles' );
}
