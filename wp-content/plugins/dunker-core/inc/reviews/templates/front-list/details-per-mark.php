<div class="qodef-reviews-list-info qodef-reviews-per-mark">
	<?php foreach ( $post_ratings as $rating ) { ?>
		<?php
		$average_rating     = dunker_core_post_average_rating( $rating );
		$rating_count       = $rating['count'];
		$rating_count_label = 1 === $rating_count ? esc_html__( 'Rating', 'dunker-core' ) : esc_html__( 'Ratings', 'dunker-core' );
		...
		<div class="qodef-reviews-number-holder">
			<div class="qodef-reviews-number-wrapper">
				...
				<span class="qodef-stars-wrapper">
							...
							<span class="qodef-reviews-count">
								<?php echo esc_html__( 'Rated', 'dunker-core' ) . ' ' . $average_rating . ' ' . esc_html__( 'out of', 'dunker-core' ) . ' ' . $rating_count . ' ' . $rating_count_label; ?>
							</span>
						</span>
			</div>
			<div class="qodef-rating-percentage-wrapper">
				...
				foreach ( $rating_marks as $item => $value ) {
					...
					echo do_shortcode( '[dunker_core_progress_bar layout="line" number="' . esc_attr( $percentage ) . '" title="' . esc_attr( $item ) . esc_attr__( ' stars', 'dunker-core' ) . '"]' );
				}
				?>
			</div>
		</div>
