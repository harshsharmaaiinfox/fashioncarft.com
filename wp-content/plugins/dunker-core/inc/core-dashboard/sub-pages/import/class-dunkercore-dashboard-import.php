...
class DunkerCore_Dashboard_Import {
	/**
	 * @var DunkerCore_Dashboard_Import of current class
	 */
	...
	/**
	 * @return DunkerCore_Dashboard_Import
	 */
	...
	public function set_import_url() {
		$params = DunkerCore_Dashboard::get_instance()->get_import_params();
				...
				switch ( $_POST['options']['action'] ) :
					...
					case 'rev-slider':
						if ( qode_framework_is_installed( 'revolution-slider' ) ) {
							...
							$this->set_message( esc_html__( 'Revolution Slider isn\'t installed', 'dunker-core' ) );
						}
						break;
					case 'content':
						...
						if ( $content_start ) {
							if ( ! DunkerCore_Dashboard::get_instance()->check_purchase_code( $_POST['options']['demo'] ) ) {
								qode_framework_get_ajax_status( 'error', esc_html__( 'Please don\'t try to hack me. Purchase code registered is not valid', 'dunker-core' ) );
								exit;
							}
						}
	...
	function file_content( $path ) {
		...
		if ( 200 !== intval( $response_code ) ) {
			$this->set_message( $response['response']['message'] . ' ' . esc_html__( 'Please contact support', 'dunker-core' ) );
	...
	public function import_widgets( $demo ) {
		...
		if ( $widgets_content ) {
			foreach ( (array) $widgets_content['widgets'] as $dunker_widget_id => $dunker_widget_data ) {
				$dunker_widget_data = $this->fix_nav_menu_widgets( $dunker_widget_data, $dunker_widget_id );
				if ( 'block' === $dunker_widget_id ) {
					$replace_hash    = array();
					$replace_encoded = array();
					foreach ( $dunker_widget_data as $dunker_widget_block => $block_value ) {
						...
						if ( strpos( $block_value['content'], 'wp:legacy-widget' ) !== false ) {
							...
							array_walk_recursive(
								$parsed_block_content[0]['innerBlocks'],
								function ( $value, $key ) use ( &$replace_encoded, &$replace_hash, $dunker_widget_block ) {
									if ( 'encoded' === $key ) {
							...
							$dunker_widget_data[ $dunker_widget_block ]['content'] = $block_value['content'];
						}
					}
				}
				update_option( 'widget_' . $dunker_widget_id, $dunker_widget_data );
			}

			$ws = $this->import_sidebars_widgets( $widgets );

			if ( $ws ) {
				$this->set_message( esc_html__( 'Widgets are set for proper sidebar', 'dunker-core' ) );
				$this->set_data( 'type', 'options' );
				$this->set_status( 'success' );
			}
		}
	...
	function fix_nav_menu_widgets( $widget_data, $widget_id ) {
		if ( 'nav_menu' === $widget_id ) {
			$imported_terms = get_transient( 'dunker_core_imported_terms' );
	...
	public function import_sidebars_widgets( $file ) {
		$dunker_sidebars = get_option( 'sidebars_widgets' );
		unset( $dunker_sidebars['array_version'] );
		$data = $this->unserialized_content( $file );

		if ( $data && is_array( $data['sidebars'] ) ) {
			$dunker_sidebars = array_merge( (array) $dunker_sidebars, (array) $data['sidebars'] );
			unset( $dunker_sidebars['wp_inactive_widgets'] );
			$dunker_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $dunker_sidebars );
			$dunker_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $dunker_sidebars );
	...
	public function import_options( $file ) {
		...
		$current_options = get_option( DUNKER_CORE_OPTIONS_NAME );

		if ( $options ) {
			if ( $current_options != $options ) {
				$result = update_option( DUNKER_CORE_OPTIONS_NAME, $options );

				if ( $result ) {
					$this->update_options_after_import( $file );
					$this->set_status( 'success' );
					$this->set_data( 'type', 'options' );
					$this->set_message( esc_html__( 'Options imported successfully', 'dunker-core' ) );

					$this->update_options_after_import( $file );

				} else {
					$this->set_status( 'error' );
					$this->set_message( esc_html__( 'Problem occurred during options import', 'dunker-core' ) );
				}
			} else {
				$this->set_status( 'success' );
				$this->set_data( 'type', 'options' );
				$this->set_message( esc_html__( 'Options are already imported', 'dunker-core' ) );
			}
		}
	}
	...
	public function import_settings_pages( $file ) {
		...
		$new_ids       = get_transient( '_dunker_core_imported_posts' );
		$fields_status = true;
		...
		if ( $pages ) {
			...
			if ( ! $fields_status ) {
				$this->set_status( 'error' );
				$this->set_message( esc_html__( 'Problem occurred during settings pages import', 'dunker-core' ) );
			} else {
				$this->set_status( 'success' );
				$this->set_data( 'type', 'options' );
				$this->set_message( esc_html__( 'Settings pages imported successfully', 'dunker-core' ) );
			}
		} else {
			$this->set_status( 'error' );
			$this->set_message( esc_html__( 'File doesn\'t exist', 'dunker-core' ) );
		}
	}
	...
	public function import_menu_settings( $file ) {
		...
		if ( $menus_data ) {
			...
			$this->set_status( 'success' );
			$this->set_data( 'type', 'options' );
			$this->set_message( esc_html__( 'Menus set for proper locations', 'dunker-core' ) );
		} else {
			$this->set_status( 'error' );
			$this->set_message( esc_html__( 'Problem during menus location set', 'dunker-core' ) );
		}
	}

	public function import_content( $file, $xml, $attachments, $post_id ) {
		ob_start();
		require_once( DUNKER_CORE_INC_PATH . '/core-dashboard/sub-pages/import/wordpress-importer.php' );
		...
		$dunker_import = new WP_Import();
		set_time_limit( 0 );

		$dunker_import->fetch_attachments = $attachments;
		$returned_value                   = $dunker_import->import( $file . $xml );

		if ( is_wp_error( $returned_value ) ) {
			$this->set_status( 'error' );
			$this->set_data( 'type', 'content' );
			$this->set_data( 'xml', $xml );
			$this->set_message( esc_html__( 'An error occurred during content import', 'dunker-core' ) );
		} else {
			$this->set_status( 'success' );
			$this->set_data( 'type', 'content' );
			$this->set_data( 'posts', $this->imported_posts );
			$this->set_message( esc_html__( 'File imported successfully', 'dunker-core' ) . ' ' . $xml );
		}
		...
		if ( $files_created ) {
			...
			if ( class_exists( 'RevSliderSliderImport' ) ) {
				...
				if ( ! empty( $import_instance ) && ! is_wp_error( $import_instance ) ) {
					...
					foreach ( $demo_rev_sliders as $demo_rev_slider ) {
						...
						if ( ! in_array( str_replace( '.zip', '', $demo_rev_slider ), $rev_sliders, true ) ) {
							...
							if ( ! $rev_slider_result['success'] ) {
								$this->set_status( 'error' );
								$this->set_message( esc_html__( 'Error while importing rev sliders', 'dunker-core' ) );
								exit;
							}
						}
					}
					...
					$this->set_status( 'success' );
					$this->set_data( 'type', 'options' );
					$this->set_message( esc_html__( 'Rev sliders imported successfully', 'dunker-core' ) );
				} else {
					$this->set_status( 'error' );
					$this->set_data( 'type', 'options' );
					$this->set_message( esc_html__( 'RevSliderSliderImport instance is empty', 'dunker-core' ) );
				}
			} else {
				$this->set_status( 'error' );
				$this->set_data( 'type', 'options' );
				$this->set_message( esc_html__( 'RevSliderSliderImport class doesn\'t exist', 'dunker-core' ) );
			}
		} else {
			$this->set_status( 'error' );
			$this->set_data( 'type', 'options' );
			$this->set_message( esc_html__( 'Files don\'t exist', 'dunker-core' ) );
		}
	}
	...
	function update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( DUNKER_CORE_OPTIONS_NAME );
			$new_global_values = str_replace( $demo_url, $url, $global_options );

			update_option( DUNKER_CORE_OPTIONS_NAME, $new_global_values );
		}
	}
	...
	function get_noimage_url( $origin_img_url ) {
		...
		return DUNKER_CORE_ASSETS_URL_PATH . '/img/' . $noimage_fname;
	}
	...
	public function populate_single_pages() {
		...
		if ( isset( $_POST ) && ! empty( $_POST ) && ! empty( $_POST['options']['demo'] ) ) {
			if ( wp_verify_nonce( $_POST['options']['nonce'], 'qodef_cd_import_nonce' ) ) {
				...
				$html = dunker_core_get_template_part( 'core-dashboard/sub-pages/import/templates', 'pages-list', '', array( 'pages' => $pages ) );

				if ( $pages ) {
					qode_framework_get_ajax_status( 'success', '', $html );
				} else {
					qode_framework_get_ajax_status( 'error', esc_html__( 'Pages don\'t exist', 'dunker-core' ), '' );
				}
			}
		}
	...
	public function is_ready_to_import() {
		$info = DunkerCore_Dashboard_System_Info_Page::get_instance()->get_system_info();
...
DunkerCore_Dashboard_Import::get_instance();
