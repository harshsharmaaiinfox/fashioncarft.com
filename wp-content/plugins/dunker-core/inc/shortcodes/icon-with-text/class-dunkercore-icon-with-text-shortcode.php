...
if ( ! function_exists( 'dunker_core_add_icon_with_text_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function dunker_core_add_icon_with_text_shortcode( $shortcodes ) {
		$shortcodes[] = 'DunkerCore_Icon_With_Text_Shortcode';

		return $shortcodes;
	}

	add_filter( 'dunker_core_filter_register_shortcodes', 'dunker_core_add_icon_with_text_shortcode' );
}

if ( class_exists( 'DunkerCore_Shortcode' ) ) {
	class DunkerCore_Icon_With_Text_Shortcode extends DunkerCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'dunker_core_filter_icon_with_text_layouts', array() ) );

			$options_map   = dunker_core_get_variations_options_map( $this->get_layouts() );
			$default_value = $options_map['default_value'];

			$this->set_extra_options( apply_filters( 'dunker_core_filter_icon_with_text_extra_options', array(), $default_value ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( DUNKER_CORE_SHORTCODES_URL_PATH . '/icon-with-text' );
			$this->set_base( 'dunker_core_icon_with_text' );
			$this->set_name( esc_html__( 'Icon With Text', 'dunker-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon with text element', 'dunker-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'dunker_core_icon_with_text', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return dunker_core_get_template_part( 'shortcodes/icon-with-text', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
		...
		private function get_holder_classes( $atts ) {
			...
			$holder_classes = apply_filters( 'dunker_core_filter_icon_with_text_variation_classes', $holder_classes, $atts );
