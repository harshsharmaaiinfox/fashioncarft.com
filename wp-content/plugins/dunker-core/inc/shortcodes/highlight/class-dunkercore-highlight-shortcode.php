...
if ( ! function_exists( 'dunker_core_add_highlight_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function dunker_core_add_highlight_shortcode( $shortcodes ) {
		$shortcodes[] = 'DunkerCore_Highlight_Shortcode';

		return $shortcodes;
	}

	add_filter( 'dunker_core_filter_register_shortcodes', 'dunker_core_add_highlight_shortcode' );
}

if ( class_exists( 'DunkerCore_Shortcode' ) ) {
	class DunkerCore_Highlight_Shortcode extends DunkerCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( DUNKER_CORE_SHORTCODES_URL_PATH . '/highlight' );
			$this->set_base( 'dunker_core_highlight' );
			$this->set_name( esc_html__( 'Highlight', 'dunker-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays highlight with provided parameters', 'dunker-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Disable Text Line Break', 'dunker-core' ),
					'description'   => esc_html__( 'Enabling this option will disable text line breaks for screen size 1024 and lower', 'dunker-core' ),
			...
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Highlight Text Position', 'dunker-core' ),
					'description' => esc_html__( 'Enter the positions of the words you would like to display as "highlight" text. Separate start and end positions with commas (e.g. if you would like to wrap from fifth to ninth words, you would enter "5,9") or if you want to highlight whole text fill -1', 'dunker-core' ),
					'group'       => esc_html__( 'Text Style', 'dunker-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Highlight Text Color', 'dunker-core' ),
					'group'      => esc_html__( 'Text Style', 'dunker-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Highlight Text Background Color', 'dunker-core' ),
					'group'      => esc_html__( 'Text Style', 'dunker-core' ),
				)
			);
			$this->set_option(
		...
		public function render( $options, $content = null ) {
			...
			return dunker_core_get_template_part( 'shortcodes/highlight', 'templates/highlight', '', $atts );
		}
