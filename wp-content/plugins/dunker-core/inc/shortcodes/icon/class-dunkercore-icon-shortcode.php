...
if ( ! function_exists( 'dunker_core_add_icon_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function dunker_core_add_icon_shortcode( $shortcodes ) {
		$shortcodes[] = 'DunkerCore_Icon_Shortcode';

		return $shortcodes;
	}

	add_filter( 'dunker_core_filter_register_shortcodes', 'dunker_core_add_icon_shortcode', 8 );
}

if ( class_exists( 'DunkerCore_Shortcode' ) ) {
	class DunkerCore_Icon_Shortcode extends DunkerCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( DUNKER_CORE_SHORTCODES_URL_PATH . '/icon' );
			$this->set_base( 'dunker_core_icon' );
			$this->set_name( esc_html__( 'Icon', 'dunker-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon element', 'dunker-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					...
					'options'    => array(
						...
						'tiny'    => esc_html__( 'Tiny', 'dunker-core' ),
						'small'   => esc_html__( 'Small', 'dunker-core' ),
						'medium'  => esc_html__( 'Medium', 'dunker-core' ),
						'large'   => esc_html__( 'Large', 'dunker-core' ),
						'huge'    => esc_html__( 'Huge', 'dunker-core' ),
					),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_size',
					'title'      => esc_html__( 'Custom Size', 'dunker-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'options'       => array(
						'normal' => esc_html__( 'Normal', 'dunker-core' ),
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'shape_size',
					'title'      => esc_html__( 'Shape Size', 'dunker-core' ),
					'group'      => esc_html__( 'Style', 'dunker-core' ),
					'dependency' => array(
						'show' => array(
							'icon_layout' => array(
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Vertical Offset', 'dunker-core' ),
					'group'      => esc_html__( 'Style', 'dunker-core' ),
				)
			);
			$this->set_option(
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'dunker_core_icon', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return dunker_core_get_template_part( 'shortcodes/icon', 'templates/icon', '', $atts );
		}
